$(document).ready(function () {
    var GET_DATA_URL='/gostinec/SupplierData', SAVE_DATA_URL='/gostinec/SupplierDataSave', FILE_DELETE='/gostinec/SupplierFileDelete',
        imgContainer,jqUploadId,jqSupplier,jqBranchesStr,first_name,last_name,contact_email,company_name,address,city,
        ...
        btnSave,btnCancel,ctrl,arr,settingsStates,jqTbl,jqRemoveAll,jqAddNew,dummyIndex = 0;

    gostinec.ui.settings = settingsStates = {
        'open' : function(){
            gostinec.log.info('State open for supplierid:' + supplier);
            imgContainer = $('.imageContainer');
            jqUploadId = $('div.uploader input[type="hidden"]#item');
            jqSupplier = $('div.settings input.supplier');
            first_name = $('form#settings input.first_name');
            ...
            btnSave = $('div.action .aShrani');
            btnCancel = $('div.action .aPreklici');
            ...
            $.ajax({
                type: "POST",
                url: GET_DATA_URL,
                data: { 'name': 'supplier', 'supplier': supplier },
                dataType: "json",
                success: function (data) {
                    if (data && data.name==='supplier' && data.aaData) {
                        gostinec.log.info('OPEN xhr Sucess');
        ...
        'bind' : function(bindData){
            ...
            gostinec.log.info('BIND state');
            jqUploadId.val(supplier);
            jqSupplier.val(supplier);
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data && data.name==='supplier' && data.aaData==='ok') {
                        gostinec.log.info('SAVE STATE - sucess saving ');
        ...
        'cancel' : function (){
            gostinec.log.info('STATE CANCEL');
            settingsStates.close();
        },
        'logo' : {
            open : function(logoUrl){
                var imgManager, imgOdstrani, img;
                gostinec.log.info('STATE LOGO OPEN for:' + logoUrl);
            ...
            'handlers' :{
                'remove' : function(){
                    gostinec.log.info('STATE LOGO HANDLER REMOVE');
                    $.ajax({
                        type: "POST",
                        url: FILE_DELETE,
                        data: {'name': 'supplier', 'supplier': supplier },
